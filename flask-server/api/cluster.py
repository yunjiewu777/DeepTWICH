...
def get_keywords():
    try:
        ...
        if os.path.exists('./model/user_model'):
            os.remove("./model/user_model")
        ...
        # Serialize the dictionary to a file using pickle
        with open('./model/no_input.pickle', 'wb') as f:
            pickle.dump(clus, f)
...
def add_constraint():
    try:
        ...
        key = anchor['word']+positive['word']+negative['word']

        db.collection("constraints").document(key).set(
...
def re_cluster():
    try:
        ...
        print('1 here', flush=True)

        two_dim = auto_encoder(r, con)

        print('4 here', flush=True)
        ...
        print(clus, flush=True)
        # Serialize the dictionary to a file using pickle
        with open('./model/input_3.pickle', 'wb') as f:
            pickle.dump(clus, f)
...
def auto_encoder(data, con):
    ...
    # load the model
    if os.path.exists('./model/user_model'):
        model.load_state_dict(torch.load('./model/user_model'))
    else:
        model.load_state_dict(torch.load('./model/init_model'))
    ...
    for epoch in range(epochs):
        ...
        for i_batch, sample_batched in enumerate(embedding):
            ...
            print('2.2 here', flush=True)


            # loss = np.average(losses)
            # losses = []

            print('2.3 here', flush=True)
    ...
    print('3 here', flush=True)


    # load the last checkpoint with the best model
    model.load_state_dict(torch.load('checkpoint.pt'))

    torch.save(model.state_dict(), './model/user_model')
